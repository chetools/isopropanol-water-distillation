...
def build_process_ledger(c: dict) -> pd.DataFrame:
    ...
    rows = [
        ...
        _row(14, "N_F", "Feed stage from top",
             "specified first stripping stage" if c.get("feed_stage_spec") is not None
             else "first stage switching from rectifying to stripping construction",
             (f"locked at stage {c['feed_stage_spec']}; feed-line crossing would be {c.get('optimal_feed_stage', c['feed_stage'])}"
              if c.get("feed_stage_spec") is not None
              else f"first x_n≤z_F={z:.12g} with implemented near-feed guard"),
             c["feed_stage"], "stage", "Exact switching condition appears in column.py."),
