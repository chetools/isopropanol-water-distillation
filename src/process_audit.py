...
def render_process_calculation_audit(column: dict) -> None:
    """Render equations, substitutions, state table, and exact implementation."""
    with st.expander("How every process KPI is calculated — equations, substitutions, and Python", expanded=False):
        st.caption(
            "KPI cards are rounded for readability. This ledger and the source-code tabs retain the calculation precision."
        )
        ledger_tab, stage_tab, code_tab = st.tabs(
            ["19-step KPI ledger", "Every calculated stage state", "Exact hand-coded Python"]
        )
        with ledger_tab:
        ...
        with stage_tab:
            ...
            st.markdown(
                ...
                "Ponchon–Savarit lever rule. The complete algorithm is in the next tab."
            )
        ...
        with code_tab:
            ...
            choices = {
                "Thermodynamics: vapor pressure, NRTL, enthalpy, bubble/dew roots": source_dir / "thermo.py",
                "Column: balances, minimum reflux, stage stepping, duties": source_dir / "column.py",
                "Specification solver: every allowed DOF pair": source_dir / "dof_manager.py",
            }
            selected = st.selectbox("Calculation module", list(choices), key="process_source_module")
            source = choices[selected].read_text(encoding="utf-8")
            st.download_button(
                f"Download {choices[selected].name}", source.encode("utf-8"), choices[selected].name, "text/x-python"
            )
